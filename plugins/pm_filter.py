...
from utils import temp, get_settings, is_check_admin, get_status, get_hash, get_size, save_group_settings, is_req_subscribed, get_poster, get_status, get_readable_time , imdb , formate_file_name , replace_words
from database.users_chats_db import db
from database.ia_filterdb import Media, get_search_results, get_bad_files, get_file_details
import random
lock = asyncio.Lock()
from .Lucy.checkFsub import is_user_fsub
import traceback
...
async def year_search(client: Client, query: CallbackQuery):
    ...
    btn.insert(0,[
        InlineKeyboardButton("• ᴄʜᴏᴏsᴇ ʟᴀɴɢᴜᴀɢᴇ •", callback_data=f"seasons#{key}#{offset}#{req}")
        ])
    btn.insert(1, [
        InlineKeyboardButton("• ǫᴜᴀʟɪᴛʏ", callback_data=f"qualities#{key}#{offset}#{req}"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        buttons = [[
            ...
            InlineKeyboardButton('• ᴍᴏʀᴇ •', callback_data='features')
        ], [
        ...
        buttons = [[
            ...
            InlineKeyboardButton('ᴘᴅғᴍᴇʀɢᴇ', callback_data='mergepdf')
        ], [
        ...
        buttons = [[
            InlineKeyboardButton('× ᴀʟʟ ᴏᴜʀ ʟɪɴᴋꜱ ×', url="https://t.me/SB_Botz_Update/262")
       ],[
            InlineKeyboardButton('• ɢʀᴏᴜᴘ •', url="https://t.me/SB_Movie_Group"),
            InlineKeyboardButton('• ᴜᴘᴅᴀᴛᴇs •', url="https://t.me/SB_Botz_Update")
       ],[
            InlineKeyboardButton('• sᴇʀɪᴇs •', url="https://t.me/apnalink_serise"),
            InlineKeyboardButton('• ᴍᴏᴠɪᴇ •', url="https://t.me/MovieVillaTG")
       ],[
            InlineKeyboardButton('• ᴀɴɪᴍᴇ •', url="https://t.me/+HC5-Y0NyVI05ODA1")
       ],[ 
...
async def auto_filter(client, msg, spoll=False , pm_mode = False):
    ...
    if not spoll:
        ...
        if not files:
            if settings["spell_check"]:
                await delSticker(st)
                ai_sts = await msg.reply_text('<b>ᴘʀᴏғᴇssᴏʀ ɪs ᴄʜᴇᴄᴋɪɴɢ ғᴏʀ ᴜʀ sᴘᴇʟʟɪɴɢ ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ..</b>')
                is_misspelled = await ai_spell_check(search)
                if is_misspelled:
                    await ai_sts.edit(f'<b>ᴘʀᴏғᴇssᴏʀ sᴜɢɢᴇsᴛᴇᴅ <code>{is_misspelled}</code>\nsᴏ ɪᴍ sᴇᴀʀᴄʜɪɴɢ ғᴏʀ <code>{is_misspelled}</code></b>')
    ...
    if offset != "":
        ...
        if total_results >= 3:
            ...
            btn.insert(0,[
                InlineKeyboardButton("❌ ɴᴏ ᴍᴏʀᴇ ᴘᴀɢᴇs ❌", user_id=ADMINS)
            ])
